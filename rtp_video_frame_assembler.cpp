...
#include "libmedia_transfer_protocol/rtp_rtcp/video_rtp_depacketizer_raw.h"
...
RtpVideoFrameAssembler::Impl::AssembleFrames(
  ...
  for (auto& packet : insert_result.packets) {
    ...
    if (packet->is_last_packet_in_frame()) {
      ...
      result.push_back(std::make_unique<libmedia_codec::RtpFrameObject>(
          ...
          /*ntp_time_ms=*/0,                      //
          /*timing=*/libmedia_codec::VideoSendTiming(),           //
...
RtpVideoFrameAssembler::Impl::FindReferences(RtpFrameVector frames) {
  ...
  for (auto& frame : frames) {
    ...
    for (std::unique_ptr<libmedia_codec::RtpFrameObject>& complete_frame : complete_frames) {
