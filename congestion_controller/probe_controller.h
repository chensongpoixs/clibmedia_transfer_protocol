...
#ifndef _C_PROBE_CONTROLLER_H_
#define _C_PROBE_CONTROLLER_H_
...
//#include "libmedia_transfer_protocol/network_control.h"
...
struct ProbeControllerConfig {
  ...
  webrtc::FieldTrialParameter<webrtc::TimeDelta> alr_probing_interval;
  webrtc::FieldTrialParameter<double> alr_probe_scale;

  // Configures the probes emitted by changed to the allocated bitrate.
  webrtc::FieldTrialOptional<double> first_allocation_probe_scale;
  webrtc::FieldTrialOptional<double> second_allocation_probe_scale;
  webrtc::FieldTrialFlag allocation_allow_further_probing;
  webrtc::FieldTrialParameter<webrtc::DataRate> allocation_probe_max;
};
...
class ProbeController {
 public:
  ...
  ABSL_MUST_USE_RESULT std::vector< libice::ProbeClusterConfig> SetBitrates(
  ...
  ABSL_MUST_USE_RESULT std::vector<libice::ProbeClusterConfig>
  OnMaxTotalAllocatedBitrate(int64_t max_total_allocated_bitrate,
                             int64_t at_time_ms);

  ABSL_MUST_USE_RESULT std::vector<libice::ProbeClusterConfig> OnNetworkAvailability(
      libice::NetworkAvailability msg);

  ABSL_MUST_USE_RESULT std::vector<libice::ProbeClusterConfig> SetEstimatedBitrate(
      int64_t bitrate_bps,
      int64_t at_time_ms);
  ...
  ABSL_MUST_USE_RESULT std::vector<libice::ProbeClusterConfig> Process(
      int64_t at_time_ms);
 ...
 private:
  ...
  ABSL_MUST_USE_RESULT std::vector<libice::ProbeClusterConfig> InitiateProbing(
      int64_t now_ms,
