...
namespace {
constexpr webrtc::TimeDelta kBweIncreaseInterval =webrtc::TimeDelta::Millis(1000);
constexpr webrtc::TimeDelta kBweDecreaseInterval =webrtc::TimeDelta::Millis(300);
 // 2s
constexpr webrtc::TimeDelta kStartPhase =			webrtc::TimeDelta::Millis(2000);
constexpr webrtc::TimeDelta kBweConverganceTime = webrtc:: TimeDelta::Millis(20000);
constexpr int kLimitNumPackets = 20;
constexpr webrtc::DataRate kDefaultMaxBitrate =		webrtc::DataRate::BitsPerSec(1000000000);
constexpr webrtc::TimeDelta kLowBitrateLogPeriod =	webrtc::TimeDelta::Millis(10000);
constexpr webrtc::TimeDelta kRtcEventLogPeriod =	webrtc::TimeDelta::Millis(5000);
...
constexpr webrtc::DataRate kDefaultBitrateThreshold = webrtc::DataRate::Zero();
...
LinkCapacityTracker::LinkCapacityTracker()
    : tracking_rate("rate", webrtc::TimeDelta::Seconds(10)) {
	webrtc::ParseFieldTrial({&tracking_rate},
		webrtc::field_trial::FindFullName("WebRTC-Bwe-LinkCapacity"));
}
...
void LinkCapacityTracker::OnStartingRate(webrtc::DataRate start_rate) {
  if (last_link_capacity_update_.IsInfinite())
    capacity_estimate_bps_ = start_rate.bps<double>();
}

void LinkCapacityTracker::OnRateUpdate(absl::optional<webrtc::DataRate> acknowledged,
                                       webrtc::DataRate target,
                                       webrtc::Timestamp at_time) {
  if (!acknowledged)
    return;
  webrtc::DataRate acknowledged_target = std::min(*acknowledged, target);
  if (acknowledged_target.bps() > capacity_estimate_bps_) {
	  webrtc::TimeDelta delta = at_time - last_link_capacity_update_;
...
void LinkCapacityTracker::OnRttBackoff(webrtc::DataRate backoff_rate,
                                       webrtc::Timestamp at_time) {
  capacity_estimate_bps_ =
...
webrtc::DataRate LinkCapacityTracker::estimate() const {
  return webrtc::DataRate::BitsPerSec(capacity_estimate_bps_);
}

RttBasedBackoff::RttBasedBackoff(/*const WebRtcKeyValueConfig* key_value_config*/)
    : disabled_("Disabled"),
      configured_limit_("limit", webrtc::TimeDelta::Seconds(3)),
      drop_fraction_("fraction", 0.8),
      drop_interval_("interval", webrtc::TimeDelta::Seconds(1)),
      bandwidth_floor_("floor", webrtc::DataRate::KilobitsPerSec(5)),
      rtt_limit_(webrtc::TimeDelta::PlusInfinity()),
      // By initializing this to plus infinity, we make sure that we never
      // trigger rtt backoff unless packet feedback is enabled.
      last_propagation_rtt_update_(webrtc::Timestamp::PlusInfinity()),
      last_propagation_rtt_(webrtc::TimeDelta::Zero()),
      last_packet_sent_(webrtc::Timestamp::MinusInfinity()) {
...
void RttBasedBackoff::UpdatePropagationRtt(webrtc::Timestamp at_time,
                                           webrtc::TimeDelta propagation_rtt) {
  last_propagation_rtt_update_ = at_time;
  last_propagation_rtt_ = propagation_rtt;
}

webrtc::TimeDelta RttBasedBackoff::CorrectedRtt(webrtc::Timestamp at_time) const {
  webrtc::TimeDelta time_since_rtt = at_time - last_propagation_rtt_update_;
  webrtc::TimeDelta timeout_correction = time_since_rtt;
  // Avoid timeout when no packets are being sent.
  webrtc::TimeDelta time_since_packet_sent = at_time - last_packet_sent_;
  timeout_correction =
      std::max(time_since_rtt - time_since_packet_sent, webrtc::TimeDelta::Zero());
...
SendSideBandwidthEstimation::SendSideBandwidthEstimation(
   // const WebRtcKeyValueConfig* key_value_config,
    ...
    : rtt_backoff_(),
      ...
      current_target_(webrtc::DataRate::Zero()),
      last_logged_target_(webrtc::DataRate::Zero()),
      min_bitrate_configured_(
		  webrtc::DataRate::BitsPerSec(congestion_controller::GetMinBitrateBps())),
      max_bitrate_configured_(kDefaultMaxBitrate),
      last_low_bitrate_log_(webrtc::Timestamp::MinusInfinity()),
      has_decreased_since_last_fraction_loss_(false),
      last_loss_feedback_(webrtc::Timestamp::MinusInfinity()),
      last_loss_packet_report_(webrtc::Timestamp::MinusInfinity()),
      last_fraction_loss_(0),
      last_logged_fraction_loss_(0),
      last_round_trip_time_(webrtc::TimeDelta::Zero()),
      receiver_limit_(webrtc::DataRate::PlusInfinity()),
      delay_based_limit_(webrtc::DataRate::PlusInfinity()),
      time_last_decrease_(webrtc::Timestamp::MinusInfinity()),
      first_report_time_(webrtc::Timestamp::MinusInfinity()),
      initially_lost_packets_(0),
      bitrate_at_2_seconds_(webrtc::DataRate::Zero()),
      uma_update_state_(kNoUpdate),
      uma_rtt_state_(kNoUpdate),
      rampup_uma_stats_updated_(kNumUmaRampupMetrics, false),
      //event_log_(event_log),
      last_rtc_event_log_(webrtc::Timestamp::MinusInfinity()),
  ...
  if (BweLossExperimentIsEnabled()) {
    ...
    if (ReadBweLossExperimentParameters(&low_loss_threshold_,
      ...
      RTC_LOG(LS_INFO) << "Enabled BweLossExperiment with parameters "
                       ...
                       << ", " << bitrate_threshold_kbps;
      bitrate_threshold_ = webrtc::DataRate::KilobitsPerSec(bitrate_threshold_kbps);
    }
  }
...
void SendSideBandwidthEstimation::SetBitrates(
    absl::optional<webrtc::DataRate> send_bitrate,
...
void SendSideBandwidthEstimation::SetSendBitrate(webrtc::DataRate bitrate,
                                                 webrtc::Timestamp at_time) {
...
void SendSideBandwidthEstimation::SetMinMaxBitrate(webrtc::DataRate min_bitrate,
                                                   webrtc::DataRate max_bitrate) {
  min_bitrate_configured_ =
...
webrtc::DataRate SendSideBandwidthEstimation::GetEstimatedLinkCapacity() const {
  return link_capacity_.estimate();
}

void SendSideBandwidthEstimation::UpdateReceiverEstimate(webrtc::Timestamp at_time,
                                                         webrtc::DataRate bandwidth) {
  // TODO(srte): Ensure caller passes PlusInfinity, not zero, to represent no
  // limitation.
  receiver_limit_ = bandwidth.IsZero() ? webrtc::DataRate::PlusInfinity() : bandwidth;
  ApplyTargetLimits(at_time);
}

void SendSideBandwidthEstimation::UpdateDelayBasedEstimate(webrtc::Timestamp at_time,
                                                           webrtc::DataRate bitrate) {
  link_capacity_.UpdateDelayBasedEstimate(at_time, bitrate);
  // TODO(srte): Ensure caller passes PlusInfinity, not zero, to represent no
  // limitation.
  delay_based_limit_ = bitrate.IsZero() ? webrtc::DataRate::PlusInfinity() : bitrate;
  ApplyTargetLimits(at_time);
}

void SendSideBandwidthEstimation::SetAcknowledgedRate(
    absl::optional<webrtc::DataRate> acknowledged_rate,
	webrtc::Timestamp at_time) {
...
void SendSideBandwidthEstimation::UpdatePacketsLost(int64_t packets_lost,
  ...
  if (number_of_packets > 0) 
  {
    ...
    if (expected < kLimitNumPackets/*20*/) {
      // Accumulate reports.
...
void SendSideBandwidthEstimation::UpdateUmaStatsPacketsLost(webrtc::Timestamp at_time,
                                                            int packets_lost) {
	webrtc::DataRate bitrate_kbps =
		webrtc::DataRate::KilobitsPerSec((current_target_.bps() + 500) / 1000);
...
void SendSideBandwidthEstimation::UpdateRtt(webrtc::TimeDelta rtt, webrtc::Timestamp at_time) {
...
void SendSideBandwidthEstimation::UpdateEstimate(webrtc::Timestamp at_time) 
{
  ...
  if (LossBasedBandwidthEstimatorV1ReadyForUse()) {
	  webrtc::DataRate new_bitrate = loss_based_bandwidth_estimator_v1_.Update(
  ...
  webrtc::TimeDelta time_since_loss_packet_report = at_time - last_loss_packet_report_;
  //根据掉包率 调整当前码流  上次时间现在在6s以内就处理
  if (time_since_loss_packet_report < 1.2 * kMaxRtcpFeedbackInterval/*5s*/) {
    // We only care about loss above a given bitrate threshold.
    float loss = last_fraction_loss_ / 256.0f;
    // We only make decisions based on loss when the bitrate is above a
    // threshold. This is a crude way of handling loss which is uncorrelated
    // to congestion.
    if (current_target_ < bitrate_threshold_ || loss <= low_loss_threshold_/*0.02f*/) {
      ...
      //   108kbps.
		// 低与 掉包率低于2%  增加码流  计算公式  = （最小码流中  * 1.08 + 0.5）;
		webrtc::DataRate new_bitrate = webrtc::DataRate::BitsPerSec(
      ...
      if (loss <= high_loss_threshold_/*0.1f*/)  
	  {
        ...
        if (!has_decreased_since_last_fraction_loss_ &&
            (at_time - time_last_decrease_) >=
                (kBweDecreaseInterval/*300ms*/ + last_round_trip_time_ /* rtt */)) {
          time_last_decrease_ = at_time;
...
void SendSideBandwidthEstimation::OnSentPacket(const libice::SentPacket& sent_packet) {
  // Only feedback-triggering packets will be reported here.
  rtt_backoff_.last_packet_sent_ = sent_packet.send_time;
}

bool SendSideBandwidthEstimation::IsInStartPhase(webrtc::Timestamp at_time) const {
  return first_report_time_.IsInfinite() ||
         at_time - first_report_time_ < kStartPhase;
}

void SendSideBandwidthEstimation::UpdateMinHistory(webrtc::Timestamp at_time) {
  // Remove old data points from history.
  // Since history precision is in ms, add one so it is able to increase
  // bitrate if it is off by as little as 0.5ms.
	//超过时间间隔1s 就删除
  while (!min_bitrate_history_.empty() &&
         at_time - min_bitrate_history_.front().first + webrtc::TimeDelta::Millis(1) >
             kBweIncreaseInterval) {
...
webrtc::DataRate SendSideBandwidthEstimation::GetUpperLimit() const 
{
	//基于延迟带宽估计码流
	webrtc::DataRate upper_limit = delay_based_limit_;
	//自己设置的最大码流
	if (disable_receiver_limit_caps_only_)
	{
		upper_limit = std::min(upper_limit, receiver_limit_);
  }
  return std::min(upper_limit, max_bitrate_configured_);// max_bitrate = 1GB
}

void SendSideBandwidthEstimation::MaybeLogLowBitrateWarning(webrtc::DataRate bitrate,
	webrtc::Timestamp at_time) {
  if (at_time - last_low_bitrate_log_ > kLowBitrateLogPeriod) {
    RTC_LOG(LS_WARNING) << "Estimated available bandwidth " << webrtc::ToString(bitrate)
                        << " is below configured min bitrate "
                        << webrtc::ToString(min_bitrate_configured_) << ".";
    last_low_bitrate_log_ = at_time;
  }
}

void SendSideBandwidthEstimation::MaybeLogLossBasedEvent(webrtc::Timestamp at_time) {
...
void SendSideBandwidthEstimation::UpdateTargetBitrate(webrtc::DataRate new_bitrate,
                                                      webrtc::Timestamp at_time) {
	// 上限和下限的码流
...
void SendSideBandwidthEstimation::ApplyTargetLimits(webrtc::Timestamp at_time) {
