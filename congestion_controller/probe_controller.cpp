...
void MaybeLogProbeClusterCreated(/*RtcEventLog* event_log,*/
                                 const libice::ProbeClusterConfig& probe) {
...
ProbeControllerConfig::ProbeControllerConfig(
    ...
    : first_exponential_probe_scale("p1", 3.0),
      ...
      alr_probing_interval("alr_interval", webrtc::TimeDelta::Seconds(5)),
      alr_probe_scale("alr_scale", 2),
      first_allocation_probe_scale("alloc_p1", 1),
      second_allocation_probe_scale("alloc_p2", 2),
      allocation_allow_further_probing("alloc_probe_further", false),
      allocation_probe_max("alloc_probe_max", webrtc::DataRate::PlusInfinity()) {
  //ParseFieldTrial(
...
ProbeController::ProbeController(/*const WebRtcKeyValueConfig* key_value_config,
    ...
    : enable_periodic_alr_probing_(false),
      in_rapid_recovery_experiment_(false/*absl::StartsWith(
          key_value_config->Lookup(kBweRapidRecoveryExperiment),
          "Enabled")*/),
      limit_probes_with_allocateable_rate_(true/*!absl::StartsWith(
...
std::vector<libice::ProbeClusterConfig> ProbeController::SetBitrates(
  ...
  return std::vector<libice::ProbeClusterConfig>();
}

std::vector<libice::ProbeClusterConfig> ProbeController::OnMaxTotalAllocatedBitrate(
  ...
  if (state_ == State::kProbingComplete &&
    ...
    webrtc::DataRate first_probe_rate =
		webrtc::DataRate::BitsPerSec(max_total_allocated_bitrate) *
        config_.first_allocation_probe_scale.Value();
	webrtc::DataRate probe_cap = config_.allocation_probe_max.Get();
    ...
    if (config_.second_allocation_probe_scale) {
		webrtc::DataRate second_probe_rate =
			webrtc::DataRate::BitsPerSec(max_total_allocated_bitrate) *
...
std::vector<libice::ProbeClusterConfig> ProbeController::OnNetworkAvailability(
...
std::vector<libice::ProbeClusterConfig> ProbeController::InitiateExponentialProbing(
    int64_t at_time_ms) {
...
std::vector<libice::ProbeClusterConfig> ProbeController::SetEstimatedBitrate(
    int64_t bitrate_bps,
    int64_t at_time_ms) {
...
std::vector<libice::ProbeClusterConfig> ProbeController::RequestProbe(
    int64_t at_time_ms) {
...
std::vector<libice::ProbeClusterConfig> ProbeController::InitiateProbing(
    int64_t now_ms,
  ...
  for (int64_t bitrate : bitrates_to_probe) {
    ...
    libice::ProbeClusterConfig config;
    config.at_time = webrtc::Timestamp::Millis(now_ms);
    config.target_data_rate =
       webrtc:: DataRate::BitsPerSec(rtc::dchecked_cast<int>(bitrate));
    config.target_duration = webrtc::TimeDelta::Millis(kMinProbeDurationMs);
