...
bool IsValid(webrtc::DataRate datarate) {
  return datarate.IsFinite();
}

bool IsValid(webrtc::Timestamp timestamp) {
...
struct PacketResultsSummary {
  ...
  webrtc::DataSize total_size = webrtc::DataSize::Zero();
  webrtc::Timestamp first_send_time =webrtc:: Timestamp::PlusInfinity();
...
PacketResultsSummary GetPacketResultsSummary(
  ...
  for (const libice::PacketResult& packet : packet_results) {
...
double GetLossProbability(double inherent_loss,
                          webrtc::DataRate loss_limited_bandwidth,
                          webrtc::DataRate sending_rate) {
...
webrtc::DataRate LossBasedBweV2::GetBandwidthEstimate() const {
  if (!IsReady()) {
    if (!IsEnabled()) {
      RTC_LOG(LS_WARNING)
...
void LossBasedBweV2::SetAcknowledgedBitrate(webrtc::DataRate acknowledged_bitrate) {
  if (IsValid(acknowledged_bitrate)) {
    ...
    RTC_LOG(LS_WARNING) << "The acknowledged bitrate must be finite: "
                        << webrtc::ToString(acknowledged_bitrate);
  }
}

void LossBasedBweV2::SetBandwidthEstimate(webrtc::DataRate bandwidth_estimate) {
  if (IsValid(bandwidth_estimate)) {
    ...
    RTC_LOG(LS_WARNING) << "The bandwidth estimate must be finite: "
                        << webrtc::ToString(bandwidth_estimate);
  }
}
...
absl::optional<LossBasedBweV2::Config> LossBasedBweV2::CreateConfig(
  ...
  webrtc::FieldTrialParameter<bool> enabled("Enabled", false);
  webrtc::FieldTrialParameter<double> bandwidth_rampup_upper_bound_factor(
      "BwRampupUpperBoundFactor", 1.1);
  webrtc::FieldTrialParameter<double> rampup_acceleration_max_factor(
      "BwRampupAccelMaxFactor", 0.0);
  webrtc::FieldTrialParameter<webrtc::TimeDelta> rampup_acceleration_maxout_time(
      "BwRampupAccelMaxoutTime", webrtc::TimeDelta::Seconds(60));
  webrtc::FieldTrialList<double> candidate_factors("CandidateFactors",
                                           {1.05, 1.0, 0.95});
  webrtc::FieldTrialParameter<double> higher_bandwidth_bias_factor("HigherBwBiasFactor",
                                                           0.00001);
  webrtc::FieldTrialParameter<double> higher_log_bandwidth_bias_factor(
      "HigherLogBwBiasFactor", 0.001);
  webrtc::FieldTrialParameter<double> inherent_loss_lower_bound(
      "InherentLossLowerBound", 1.0e-3);
  webrtc::FieldTrialParameter<webrtc::DataRate> inherent_loss_upper_bound_bandwidth_balance(
      "InherentLossUpperBoundBwBalance", webrtc::DataRate::KilobitsPerSec(15.0));
  webrtc::FieldTrialParameter<double> inherent_loss_upper_bound_offset(
      "InherentLossUpperBoundOffset", 0.05);
  webrtc::FieldTrialParameter<double> initial_inherent_loss_estimate(
      "InitialInherentLossEstimate", 0.01);
 webrtc:: FieldTrialParameter<int> newton_iterations("NewtonIterations", 1);
 webrtc:: FieldTrialParameter<double> newton_step_size("NewtonStepSize", 0.5);
 webrtc:: FieldTrialParameter<bool> append_acknowledged_rate_candidate(
      "AckedRateCandidate", true);
 webrtc::FieldTrialParameter<bool> append_delay_based_estimate_candidate(
      "DelayBasedCandidate", false);
 webrtc::FieldTrialParameter<webrtc::TimeDelta> observation_duration_lower_bound(
      "ObservationDurationLowerBound", webrtc::TimeDelta::Seconds(1));
 webrtc::FieldTrialParameter<int> observation_window_size("ObservationWindowSize", 20);
 webrtc::FieldTrialParameter<double> sending_rate_smoothing_factor(
      "SendingRateSmoothingFactor", 0.0);
 webrtc::FieldTrialParameter<double> instant_upper_bound_temporal_weight_factor(
      "InstantUpperBoundTemporalWeightFactor", 0.99);
 webrtc::FieldTrialParameter<webrtc::DataRate> instant_upper_bound_bandwidth_balance(
      "InstantUpperBoundBwBalance", webrtc::DataRate::KilobitsPerSec(15.0));
 webrtc::FieldTrialParameter<double> instant_upper_bound_loss_offset(
      "InstantUpperBoundLossOffset", 0.05);
 webrtc::FieldTrialParameter<double> temporal_weight_factor("TemporalWeightFactor",
                                                     0.99);
...
bool LossBasedBweV2::IsConfigValid() const {
  ...
  if (config_->rampup_acceleration_maxout_time <= webrtc::TimeDelta::Zero()) {
    RTC_LOG(LS_WARNING)
  ...
  if (config_->observation_duration_lower_bound <= webrtc::TimeDelta::Zero()) {
    RTC_LOG(LS_WARNING)
  ...
  if (config_->instant_upper_bound_bandwidth_balance <= webrtc::DataRate::Zero()) {
    RTC_LOG(LS_WARNING)
...
webrtc::DataRate LossBasedBweV2::GetCandidateBandwidthUpperBound() const {
  if (!acknowledged_bitrate_.has_value())
    return webrtc::DataRate::PlusInfinity();

  webrtc::DataRate candidate_bandwidth_upper_bound =
      config_->bandwidth_rampup_upper_bound_factor * (*acknowledged_bitrate_);

  if (config_->rampup_acceleration_max_factor > 0.0) {
    const webrtc::TimeDelta time_since_bandwidth_reduced = std::min(
        config_->rampup_acceleration_maxout_time,
        std::max(webrtc::TimeDelta::Zero(), last_send_time_most_recent_observation_ -
...
std::vector<LossBasedBweV2::ChannelParameters> LossBasedBweV2::GetCandidates(
  ...
  std::vector<webrtc::DataRate> bandwidths;
  ...
  const webrtc::DataRate candidate_bandwidth_upper_bound =
...
double LossBasedBweV2::GetInherentLossUpperBound(webrtc::DataRate bandwidth) const {
  if (bandwidth.IsZero()) {
    return 1.0;
  }
...
double LossBasedBweV2::GetHighBandwidthBias(webrtc::DataRate bandwidth) const {
...
webrtc::DataRate LossBasedBweV2::GetSendingRate(
	webrtc::DataRate instantaneous_sending_rate) const {
  if (num_observations_ <= 0) {
  ...
  webrtc::DataRate sending_rate_previous_observation =
...
webrtc::DataRate LossBasedBweV2::GetInstantUpperBound() const {
  return cached_instant_upper_bound_.value_or(webrtc::DataRate::PlusInfinity());
}

void LossBasedBweV2::CalculateInstantUpperBound() {
	webrtc::DataRate instant_limit = webrtc::DataRate::PlusInfinity();
...
bool LossBasedBweV2::PushBackObservation(
  ...
  const webrtc::Timestamp last_send_time = packet_results_summary.last_send_time;
  const webrtc::TimeDelta observation_duration =
