...
#include "libmedia_transfer_protocol/congestion_controller/robust_throughput_estimator.h"
...
void RobustThroughputEstimator::IncomingPacketFeedbackVector(
    const std::vector<libice::PacketResult>& packet_feedback_vector) {
...
absl::optional<webrtc::DataRate> RobustThroughputEstimator::bitrate() const {
  if (window_.size() < settings_.initial_packets)
    return absl::nullopt;

  webrtc::TimeDelta largest_recv_gap(webrtc::TimeDelta::Millis(0));
  webrtc::TimeDelta second_largest_recv_gap(webrtc  ::TimeDelta::Millis(0));
  for (size_t i = 1; i < window_.size(); i++) {
    // Find receive time gaps
    webrtc::TimeDelta gap = window_[i].receive_time - window_[i - 1].receive_time;
  ...
  webrtc::Timestamp min_send_time = window_[0].sent_packet.send_time;
  webrtc::Timestamp max_send_time = window_[0].sent_packet.send_time;
  webrtc::Timestamp min_recv_time = window_[0].receive_time;
  webrtc::Timestamp max_recv_time = window_[0].receive_time;
  webrtc::DataSize data_size = webrtc::DataSize::Bytes(0);
  ...
  webrtc::DataSize recv_size = data_size;
  webrtc::DataSize send_size = data_size;
  if (settings_.assume_shared_link) {
    ...
    // size.
    webrtc::DataSize first_last_average_size =
  ...
  // or the average gap.
  webrtc::TimeDelta send_duration = max_send_time - min_send_time;
  webrtc::TimeDelta recv_duration = (max_recv_time - min_recv_time) - largest_recv_gap;
  ...
  send_duration = std::max(send_duration, webrtc::TimeDelta::Millis(1));
  recv_duration = std::max(recv_duration, webrtc::TimeDelta::Millis(1));
