...
#include "libmedia_transfer_protocol/congestion_controller/probe_bitrate_estimator.h"
...
// expected to last.
constexpr webrtc::TimeDelta kMaxClusterHistory = webrtc::TimeDelta::Seconds(1);

// The maximum time interval between first and the last probe on a cluster
// on the sender side as well as the receive side.
constexpr webrtc::TimeDelta kMaxProbeInterval = webrtc::TimeDelta::Seconds(1);
...
absl::optional<webrtc::DataRate> ProbeBitrateEstimator::HandleProbeAndEstimateBitrate(
    const libice::PacketResult& packet_feedback) {
  int cluster_id = packet_feedback.sent_packet.pacing_info.probe_cluster_id;
  RTC_DCHECK_NE(cluster_id, libice::PacedPacketInfo::kNotAProbe);
...
ProbeBitrateEstimator::FetchAndResetLastEstimatedBitrate() {
  absl::optional<webrtc::DataRate> estimated_data_rate = estimated_data_rate_;
