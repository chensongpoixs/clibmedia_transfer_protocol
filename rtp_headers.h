...
#ifndef _C_API_RTP_HEADERS_H_
#define _C_API_RTP_HEADERS_H_
...
struct RTPHeaderExtension {
  ...
  webrtc::Timestamp GetAbsoluteSendTimestamp() const {
    RTC_DCHECK(hasAbsoluteSendTime);
    RTC_DCHECK(absoluteSendTime < (1ul << 24));
    return webrtc::Timestamp::Micros((absoluteSendTime * 1000000ll) /
                             (1 << kAbsSendTimeFraction));
  }

  webrtc::TimeDelta GetAbsoluteSendTimeDelta(uint32_t previous_sendtime) const {
    RTC_DCHECK(hasAbsoluteSendTime);
    RTC_DCHECK(absoluteSendTime < (1ul << 24));
