...
RtpSeqNumOnlyRefFinder::ManageFrameInternal(libmedia_codec::RtpFrameObject* frame) {
  ...
  if (frame->frame_type() == libmedia_codec::VideoFrameType::kVideoFrameDelta) {
  ...
  RTC_DCHECK(webrtc::AheadOrAt(frame->last_seq_num(), seq_num_it->first));

  // Since keyframes can cause reordering we can't simply assign the
  // picture id according to some incrementing counter.
  frame->SetId(frame->last_seq_num());
  frame->num_references =
      frame->frame_type() == libmedia_codec::VideoFrameType::kVideoFrameDelta;
