...
#ifndef _C_MODULES_PACING_BITRATE_PROBER_H_
#define _C_MODULES_PACING_BITRATE_PROBER_H_
...
struct BitrateProberConfig {
  ...
  webrtc::FieldTrialParameter<int> min_probe_packets_sent;
  // A minimum interval between probes to allow scheduling to be feasible.
  webrtc::FieldTrialParameter< webrtc::TimeDelta> min_probe_delta;
  // The minimum probing duration.
  webrtc::FieldTrialParameter< webrtc::TimeDelta> min_probe_duration;
  // Maximum amount of time each probe can be delayed.
  webrtc::FieldTrialParameter< webrtc::TimeDelta> max_probe_delay;
  // If NextProbeTime() is called with a delay higher than specified by
  // `max_probe_delay`, abort it.
  webrtc::FieldTrialParameter<bool> abort_delayed_probes;
};
...
class BitrateProber {
 public:
  ...
  webrtc::Timestamp NextProbeTime(webrtc::Timestamp now) const;

  // Information about the current probing cluster.
  absl::optional<libice::PacedPacketInfo> CurrentCluster(webrtc::Timestamp now);

  // Returns the minimum number of bytes that the prober recommends for
  // the next probe, or zero if not probing.
  webrtc::DataSize RecommendedMinProbeSize() const;
  ...
  void ProbeSent(webrtc::Timestamp now, webrtc::DataSize size);
 ...
 private:
  ...
  struct ProbeCluster {
	  libice::PacedPacketInfo pace_info;

    int sent_probes = 0;
    int sent_bytes = 0;
	webrtc::Timestamp created_at = webrtc::Timestamp::MinusInfinity();
	webrtc::Timestamp started_at = webrtc::Timestamp::MinusInfinity();
    int retries = 0;
  };

  webrtc::Timestamp CalculateNextProbeTime(const ProbeCluster& cluster) const;
  ...
  webrtc::Timestamp next_probe_time_;
