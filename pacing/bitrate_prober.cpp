...
constexpr webrtc::DataSize kMinProbePacketSize = webrtc::DataSize::Bytes(200);

constexpr webrtc::TimeDelta kProbeClusterTimeout = webrtc::TimeDelta::Seconds(5);
...
BitrateProberConfig::BitrateProberConfig(
   /* const WebRtcKeyValueConfig* key_value_config*/)
    : min_probe_packets_sent("min_probe_packets_sent", 5),
      min_probe_delta("min_probe_delta", webrtc::TimeDelta::Millis(1)),
      min_probe_duration("min_probe_duration", webrtc::TimeDelta::Millis(15)),
      max_probe_delay("max_probe_delay", webrtc::TimeDelta::Millis(10)),
...
BitrateProber::BitrateProber(/*const WebRtcKeyValueConfig& field_trials*/)
    : probing_state_(ProbingState::kDisabled),
      next_probe_time_(webrtc::Timestamp::PlusInfinity()),
...
void BitrateProber::CreateProbeCluster(webrtc::DataRate bitrate,
...
webrtc::Timestamp BitrateProber::NextProbeTime(webrtc::Timestamp now) const {
...
absl::optional<libice::PacedPacketInfo> BitrateProber::CurrentCluster(webrtc::Timestamp now) {
  ...
  libice::PacedPacketInfo info = clusters_.front().pace_info;
...
// feasible.
webrtc::DataSize BitrateProber::RecommendedMinProbeSize() const {
  if (clusters_.empty()) {
    return webrtc::DataSize::Zero();
  }
  webrtc::DataRate send_rate =
	  webrtc::DataRate::BitsPerSec(clusters_.front().pace_info.send_bitrate_bps);
  return 2 * send_rate * config_.min_probe_delta;
}

void BitrateProber::ProbeSent(webrtc::Timestamp now, webrtc::DataSize size) {
...
webrtc::Timestamp BitrateProber::CalculateNextProbeTime(
  ...
  webrtc::DataSize sent_bytes = webrtc::DataSize::Bytes(cluster.sent_bytes);
  webrtc::DataRate send_bitrate =
	  webrtc::DataRate::BitsPerSec(cluster.pace_info.send_bitrate_bps);
