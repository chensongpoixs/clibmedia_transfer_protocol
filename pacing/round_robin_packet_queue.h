...
#ifndef _C_MODULES_PACING_ROUND_ROBIN_PACKET_QUEUE_H_
#define _C_MODULES_PACING_ROUND_ROBIN_PACKET_QUEUE_H_
...
class RoundRobinPacketQueue {
 public:
  ...
  // returns nullopt.
  absl::optional<webrtc::Timestamp> LeadingAudioPacketEnqueueTime() const;

  webrtc::Timestamp OldestEnqueueTime() const;
  webrtc::TimeDelta AverageQueueTime() const;
  void UpdateQueueTime(webrtc::Timestamp now);
  void SetPauseState(bool paused, webrtc::Timestamp now);
 ...
 private:
  struct QueuedPacket {
   public:
    ...
    uint32_t Ssrc() const;
	webrtc::Timestamp EnqueueTime() const;
    bool IsRetransmission() const;
    uint64_t EnqueueOrder() const;
    RtpPacketToSend* RtpPacket() const;

    std::multiset<webrtc::Timestamp>::iterator EnqueueTimeIterator() const;
    void UpdateEnqueueTimeIterator(std::multiset<webrtc::Timestamp>::iterator it);
    void SubtractPauseTime(webrtc::TimeDelta pause_time_sum);

   private:
    int priority_;
	webrtc::Timestamp enqueue_time_;  // Absolute time of pacer queue entry.
    uint64_t enqueue_order_;
    bool is_retransmission_;  // Cached for performance.
    std::multiset<webrtc::Timestamp>::iterator enqueue_time_it_;
  ...
  webrtc::DataSize PacketSize(const QueuedPacket& packet) const;
  ...
  webrtc::Timestamp time_last_updated_;

  bool paused_;
  size_t size_packets_;
  webrtc::DataSize size_;
  webrtc::DataSize max_size_;
  webrtc::TimeDelta queue_time_sum_;
  webrtc::TimeDelta pause_time_sum_;
  ...
  std::multiset<webrtc::Timestamp> enqueue_times_;
