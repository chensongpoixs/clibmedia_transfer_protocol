...
#include "libmedia_transfer_protocol/pacing/paced_sender.h"
...
PacedSender::PacedSender(webrtc::Clock* clock,
...
void PacedSender::CreateProbeCluster(webrtc::DataRate bitrate, int cluster_id) {
...
void PacedSender::SetCongestionWindow(webrtc::DataSize congestion_window_size) {
  {
		webrtc::MutexLock lock(&mutex_);
...
void PacedSender::UpdateOutstandingData(webrtc::DataSize outstanding_data) {
  {
		webrtc::MutexLock lock(&mutex_);
...
void PacedSender::SetPacingRates(webrtc::DataRate pacing_rate, webrtc::DataRate padding_rate) {
  {
		webrtc::MutexLock lock(&mutex_);
...
absl::optional<webrtc::Timestamp> PacedSender::FirstSentPacketTime() const {
	webrtc::MutexLock lock(&mutex_);
...
int64_t PacedSender::TimeUntilNextProcess() {
	...
	webrtc::TimeDelta sleep_time =
      std::max(webrtc::TimeDelta::Zero(), next_send_time - clock_->CurrentTime());
...
void PacedSender::ProcessThreadAttached(webrtc::ProcessThread* process_thread) {
