...
constexpr webrtc::TimeDelta kDefaultMinPacketLimit = webrtc::TimeDelta::Millis(5);
constexpr webrtc::TimeDelta kCongestedPacketInterval = webrtc::TimeDelta::Millis(500);
// TODO(sprang): Consider dropping this limit.
// The maximum debt level, in terms of time, capped when sending packets.
constexpr webrtc::TimeDelta kMaxDebtInTime = webrtc::TimeDelta::Millis(500);
constexpr webrtc::TimeDelta kMaxElapsedTime = webrtc::TimeDelta::Seconds(2);

// Upper cap on process interval, in case process has not been called in a long
// time. Applies only to periodic mode.
constexpr webrtc::TimeDelta kMaxProcessingInterval = webrtc::TimeDelta::Millis(30);
...
// millisecond.
constexpr webrtc::TimeDelta kMaxEarlyProbeProcessing = webrtc::TimeDelta::Millis(1);
...
webrtc::TimeDelta GetDynamicPaddingTarget(/*const WebRtcKeyValueConfig& field_trials*/) {
//	webrtc::FieldTrialParameter<webrtc::TimeDelta> padding_target("timedelta",
	//  webrtc::TimeDelta::Millis(5));
 // webrtc::ParseFieldTrial({&padding_target},
...
const webrtc::TimeDelta PacingController::kMaxExpectedQueueLength =
webrtc::TimeDelta::Millis(2000);
const float PacingController::kDefaultPaceMultiplier = 2.5f;
const webrtc::TimeDelta PacingController::kPausedProcessInterval =
    kCongestedPacketInterval;
const webrtc::TimeDelta PacingController::kMinSleepTime = webrtc::TimeDelta::Millis(1);
...
PacingController::PacingController(webrtc::Clock* clock,
    ...
    : mode_(mode),
     ...
     // fallback_field_trials_(std::make_unique<FieldTrialBasedConfig>()
    ...
    //  field_trials_(/*field_trials ? field_trials : fallback_field_trials_.get()*/),
      ...
      media_budget_(0),
      padding_budget_(0),
      media_debt_(webrtc::DataSize::Zero()),
      padding_debt_(webrtc::DataSize::Zero()),
      media_rate_(webrtc::DataRate::Zero()),
      padding_rate_(webrtc::DataRate::Zero()),
      prober_(/**field_trials_*/),
      probing_send_failure_(false),
      pacing_bitrate_(webrtc::DataRate::BitsPerSec(10000)),
      last_process_time_(clock->CurrentTime()),
      last_send_time_(last_process_time_),
      packet_queue_(last_process_time_/*, field_trials_*/),
      packet_counter_(0),
      congestion_window_size_(webrtc::DataSize::PlusInfinity()),
      outstanding_data_(webrtc::DataSize::Zero()),
...
void PacingController::SetCongestionWindow(webrtc::DataSize congestion_window_size) {
  const bool was_congested = Congested();
  congestion_window_size_ = congestion_window_size;
  if (was_congested && !Congested()) {
	  webrtc::TimeDelta elapsed_time = UpdateTimeAndGetElapsed(CurrentTime());
    UpdateBudgetWithElapsedTime(elapsed_time);
  }
}

void PacingController::UpdateOutstandingData(webrtc::DataSize outstanding_data) {
  const bool was_congested = Congested();
...
webrtc::Timestamp PacingController::CurrentTime() const {
	webrtc::Timestamp time = clock_->CurrentTime();
...
void PacingController::SetPacingRates(webrtc::DataRate pacing_rate,
	webrtc::DataRate padding_rate) {
  RTC_DCHECK_GT(pacing_rate, webrtc::DataRate::Zero());
...
void PacingController::EnqueuePacket(std::unique_ptr<RtpPacketToSend> packet) {
  RTC_DCHECK(pacing_bitrate_ > webrtc::DataRate::Zero())
...
void PacingController::SetTransportOverhead(webrtc::DataSize overhead_per_packet) {
...
webrtc::TimeDelta PacingController::ExpectedQueueTime() const {
  RTC_DCHECK_GT(pacing_bitrate_, webrtc::DataRate::Zero());
  return webrtc::TimeDelta::Millis(
...
webrtc::DataSize PacingController::QueueSizeData() const {
  return packet_queue_.Size();
}

webrtc::DataSize PacingController::CurrentBufferLevel() const {
  return std::max(media_debt_, padding_debt_);
}

absl::optional<webrtc::Timestamp> PacingController::FirstSentPacketTime() const {
  return first_sent_packet_time_;
}

webrtc::TimeDelta PacingController::OldestPacketWaitTime() const {
	webrtc::Timestamp oldest_packet = packet_queue_.OldestEnqueueTime();
...
void PacingController::EnqueuePacketInternal(
    ...
    int priority) {
  prober_.OnIncomingPacket(webrtc::DataSize::Bytes(packet->payload_size()));

  webrtc::Timestamp now = CurrentTime();
  ...
  if (mode_ == ProcessMode::kDynamic && packet_queue_.Empty()) {
    ...
    // packet.
	  webrtc::Timestamp target_process_time = now;
	  webrtc::Timestamp next_send_time = NextSendTime();
	...
	webrtc::TimeDelta elapsed_time = UpdateTimeAndGetElapsed(target_process_time);
    UpdateBudgetWithElapsedTime(elapsed_time);
...
bool PacingController::ShouldSendKeepalive(webrtc::Timestamp now) const {
  if (send_padding_if_silent_ || paused_ || Congested() ||
      packet_counter_ == 0) {
    // We send a padding packet every 500 ms to ensure we won't get stuck in
    // congested state due to no feedback being received.
	  webrtc::TimeDelta elapsed_since_last_send = now - last_send_time_;
...
webrtc::Timestamp PacingController::NextSendTime() const {
  const webrtc::Timestamp now = CurrentTime();
  ...
  if (prober_.is_probing()) {
	  webrtc::Timestamp probe_time = prober_.NextProbeTime(now);
    // `probe_time` == PlusInfinity indicates no probe scheduled.
    if (probe_time != webrtc::Timestamp::PlusInfinity() && !probing_send_failure_) {
      return probe_time;
    }
  }
  ...
  if (!pace_audio_) {
    ...
    absl::optional<webrtc::Timestamp> audio_enqueue_time =
  ...
  if (media_rate_ > webrtc::DataRate::Zero() && !packet_queue_.Empty()) {
  ...
  if (padding_rate_ > webrtc::DataRate::Zero() && packet_queue_.Empty()) {
	  webrtc::TimeDelta drain_time =
...
void PacingController::ProcessPackets() {
  ...
  if (mode_ == ProcessMode::kDynamic) {
	...
	webrtc::TimeDelta early_execute_margin =
        prober_.is_probing() ? kMaxEarlyProbeProcessing : webrtc::TimeDelta::Zero();
  ...
  if (ShouldSendKeepalive(now)) {
    ...
    if (packet_counter_ == 0) {
      last_send_time_ = now;
		...
		webrtc::DataSize keepalive_data_sent = webrtc::DataSize::Zero();
      std::vector<std::unique_ptr<RtpPacketToSend>> keepalive_packets =
          packet_sender_->GeneratePadding(webrtc::DataSize::Bytes(1));
      for (auto& packet : keepalive_packets) {
        keepalive_data_sent +=
			webrtc::DataSize::Bytes(packet->payload_size() + packet->padding_size());
        packet_sender_->SendPacket(std::move(packet), libice::PacedPacketInfo());
  ...
  if (elapsed_time > webrtc::TimeDelta::Zero()) {
    ...
    if (queue_size_data > webrtc::DataSize::Zero()) {
      ...
      if (drain_large_queues_) {
		  ...
		  webrtc::DataRate min_rate_needed = queue_size_data / avg_time_left;
  ...
  if (is_probing) {
    ...
    pacing_info = prober_.CurrentCluster(now).value_or(libice::PacedPacketInfo());
    if (pacing_info.probe_cluster_id != libice::PacedPacketInfo::kNotAProbe) {
      first_packet_in_probe = pacing_info.probe_cluster_bytes_sent == 0;
      recommended_probe_size = prober_.RecommendedMinProbeSize();
      RTC_DCHECK_GT(recommended_probe_size, webrtc::DataSize::Zero());
    } else {
  ...
  while (!paused_) {
	...
	webrtc::DataSize packet_size = webrtc::DataSize::Bytes(rtp_packet->payload_size() +
  ...
  if (is_probing) {
    probing_send_failure_ = data_sent == webrtc::DataSize::Zero();
...
webrtc::DataSize PacingController::PaddingToAdd(webrtc::DataSize recommended_probe_size,
  ...
  if (mode_ == ProcessMode::kPeriodic) {
    return webrtc::DataSize::Bytes(padding_budget_.bytes_remaining());
  } else if (padding_rate_ > webrtc::DataRate::Zero() &&
             padding_debt_ == webrtc::DataSize::Zero()) {
...
std::unique_ptr<RtpPacketToSend> PacingController::GetPendingPacket(
  ...
  bool is_probe = pacing_info.probe_cluster_id != libice::PacedPacketInfo::kNotAProbe;
    ...
    if (mode_ == ProcessMode::kPeriodic) {
      ...
      if (now <= target_send_time) {
        ...
        // is not more than would be reduced to zero at the target sent time.
		  webrtc::TimeDelta flush_time = media_debt_ / media_rate_;
...
void PacingController::OnPaddingSent(webrtc::DataSize data_sent) {
  if (data_sent > webrtc::DataSize::Zero()) {
...
void PacingController::UpdateBudgetWithElapsedTime(webrtc::TimeDelta delta) {
  if (mode_ == ProcessMode::kPeriodic) {
...
void PacingController::UpdateBudgetWithSentData(webrtc::DataSize size) {
...
void PacingController::SetQueueTimeLimit(webrtc::TimeDelta limit) {
