...
void PacketBuffer::ClearTo(uint16_t seq_num) {
  ...
  if (is_cleared_to_first_seq_num_ &&
      webrtc::AheadOf<uint16_t>(first_seq_num_, seq_num)) {
    return;
  }
  ...
  ++seq_num;
  size_t diff = webrtc::ForwardDiff<uint16_t>(first_seq_num_, seq_num);
  size_t iterations = std::min(diff, buffer_.size());
  for (size_t i = 0; i < iterations; ++i) {
    auto& stored = buffer_[first_seq_num_ % buffer_.size()];
    if (stored != nullptr && webrtc::AheadOf<uint16_t>(seq_num, stored->seq_num)) {
      stored = nullptr;
    }
    ++first_seq_num_;
  }
...
std::vector<std::unique_ptr<PacketBuffer::Packet>> PacketBuffer::FindFrames(
  ...
  for (size_t i = 0; i < buffer_.size() && PotentialNewFrame(seq_num); ++i) {
    ...
    if (buffer_[index]->is_last_packet_in_frame()) {
      ...
      bool is_h264 = buffer_[start_index]->codec() == libmedia_codec::kVideoCodecH264;
      ...
      while (true) {
        ...
        if (is_h264) {
          const auto* h264_header = absl::get_if<webrtc::RTPVideoHeaderH264>(
              &buffer_[start_index]->video_header.video_type_header);
          if (!h264_header || h264_header->nalus_length >= webrtc::kMaxNalusPerPacket)
            return found_frames;

          for (size_t j = 0; j < h264_header->nalus_length; ++j) {
            if (h264_header->nalus[j].type == webrtc::H264::NaluType::kSps) {
              has_h264_sps = true;
            } else if (h264_header->nalus[j].type == webrtc::H264::NaluType::kPps) {
              has_h264_pps = true;
            } else if (h264_header->nalus[j].type == webrtc::H264::NaluType::kIdr) {
              has_h264_idr = true;
            }
          }
