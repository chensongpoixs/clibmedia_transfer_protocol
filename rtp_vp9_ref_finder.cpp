...
RtpVp9RefFinder::FrameDecision RtpVp9RefFinder::ManageFrameInternal(
    libmedia_codec::RtpFrameObject* frame) {
  const RTPVideoHeader& video_header = frame->GetRtpVideoHeader();
  const webrtc::RTPVideoHeaderVP9& codec_header =
  ...
  if (codec_header.temporal_idx >= kMaxTemporalLayers ||
      codec_header.spatial_idx >= libmedia_codec:: kMaxSpatialLayers)
    return kDrop;
  ...
  if (codec_header.flexible_mode) {
    if (codec_header.num_ref_pics > libmedia_codec::EncodedFrame::kMaxFrameReferences) {
      return kDrop;
    }
  ...
  if (codec_header.tl0_pic_idx == webrtc::kNoTl0PicIdx) {
  ...
  if (codec_header.ss_data_available) {
    if (codec_header.temporal_idx != 0) {
      ...
      if (codec_header.gof.num_frames_in_gof > webrtc::kMaxVp9FramesInGof) {
        return kDrop;
      }

      for (size_t i = 0; i < codec_header.gof.num_frames_in_gof; ++i) {
        if (codec_header.gof.num_ref_pics[i] > webrtc::kMaxVp9RefPics) {
          return kDrop;
        }
      }

	  webrtc::GofInfoVP9 gof = codec_header.gof;
    ...
    if (frame->frame_type() == libmedia_codec::VideoFrameType::kVideoFrameKey) {
      frame->num_references = 0;
      FrameReceivedVp9(frame->Id(), info);
      FlattenFrameIdAndRefs(frame, codec_header.inter_layer_predicted);
      return kHandOff;
    }
  } else if (frame->frame_type() == libmedia_codec::VideoFrameType::kVideoFrameKey) {
  ...
  if (info->gof->num_ref_pics[gof_idx] > libmedia_codec::EncodedFrame::kMaxFrameReferences) {
    return kDrop;
  }
...
void RtpVp9RefFinder::FrameReceivedVp9(uint16_t picture_id, GofInfo* info) {
  ...
  if (webrtc::AheadOf<uint16_t, kFrameIdLength>(picture_id, last_picture_id)) {
    ...
    while (last_picture_id != picture_id) {
      ...
      RTC_CHECK(gof_idx < webrtc::kMaxVp9FramesInGof);
...
void RtpVp9RefFinder::FlattenFrameIdAndRefs(libmedia_codec::RtpFrameObject* frame,
                                            bool inter_layer_predicted) {
  for (size_t i = 0; i < frame->num_references; ++i) {
    frame->references[i] =
        unwrapper_.Unwrap(frame->references[i]) * libmedia_codec:: kMaxSpatialLayers +
        *frame->SpatialIndex();
  }
  frame->SetId(unwrapper_.Unwrap(frame->Id()) * libmedia_codec::kMaxSpatialLayers +
               *frame->SpatialIndex());

  if (inter_layer_predicted &&
      frame->num_references + 1 <= libmedia_codec::EncodedFrame::kMaxFrameReferences) {
