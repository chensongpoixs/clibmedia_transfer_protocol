...
#ifndef _C_MODULES_RTP_RTCP_INCLUDE_RTP_RTCP_DEFINES_H_
#define _C_MODULES_RTP_RTCP_INCLUDE_RTP_RTCP_DEFINES_H_
...
class RtpSendRates {
  ...
  constexpr std::array<webrtc::DataRate, sizeof...(Is)> make_zero_array(
      std::index_sequence<Is...>) {
    return {{(static_cast<void>(Is), webrtc::DataRate::Zero())...}};
  }
 ...
 public:
  ...
  webrtc::DataRate& operator[](RtpPacketMediaType type) {
    return send_rates_[static_cast<size_t>(type)];
  }
  const webrtc::DataRate& operator[](RtpPacketMediaType type) const {
    return send_rates_[static_cast<size_t>(type)];
  }
  webrtc::DataRate Sum() const {
    return absl::c_accumulate(send_rates_, webrtc::DataRate::Zero());
  }

 private:
  std::array<webrtc::DataRate, kNumMediaTypes> send_rates_;
};
