...
uint8_t TemporalIdxField(const webrtc::RTPVideoHeaderVP9& hdr, uint8_t def) {
  return (hdr.temporal_idx == webrtc::kNoTemporalIdx) ? def : hdr.temporal_idx;
}

uint8_t SpatialIdxField(const webrtc::RTPVideoHeaderVP9& hdr, uint8_t def) {
  return (hdr.spatial_idx == webrtc::kNoSpatialIdx) ? def : hdr.spatial_idx;
}

int16_t Tl0PicIdxField(const webrtc::RTPVideoHeaderVP9& hdr, uint8_t def) {
  return (hdr.tl0_pic_idx == webrtc::kNoTl0PicIdx) ? def : hdr.tl0_pic_idx;
}
...
//      +-+-+-+-+-+-+-+-+
//
size_t PictureIdLength(const webrtc::RTPVideoHeaderVP9& hdr) {
  if (hdr.picture_id == webrtc::kNoPictureId)
    return 0;
  return (hdr.max_picture_id == webrtc::kMaxOneBytePictureId) ? 1 : 2;
}

bool PictureIdPresent(const webrtc::RTPVideoHeaderVP9& hdr) {
...
//                          +-+-+-+-+-+-+-+-+
//
size_t LayerInfoLength(const webrtc::RTPVideoHeaderVP9& hdr) {
  if (hdr.temporal_idx == webrtc::kNoTemporalIdx && hdr.spatial_idx == webrtc::kNoSpatialIdx) {
    return 0;
  }
  return hdr.flexible_mode ? 1 : 2;
}

bool LayerInfoPresent(const webrtc::RTPVideoHeaderVP9& hdr) {
...
//                                                current P_DIFF.
//
size_t RefIndicesLength(const webrtc::RTPVideoHeaderVP9& hdr) {
  if (!hdr.inter_pic_predicted || !hdr.flexible_mode)
    return 0;

  RTC_DCHECK_GT(hdr.num_ref_pics, 0U);
  RTC_DCHECK_LE(hdr.num_ref_pics, webrtc::kMaxVp9RefPics);
...
//      +-+-+-+-+-+-+-+-+              -|           -|
//
size_t SsDataLength(const webrtc::RTPVideoHeaderVP9& hdr) {
  if (!hdr.ss_data_available)
    return 0;

  RTC_DCHECK_GT(hdr.num_spatial_layers, 0U);
  RTC_DCHECK_LE(hdr.num_spatial_layers, webrtc::kMaxVp9NumberOfSpatialLayers);
  RTC_DCHECK_LE(hdr.gof.num_frames_in_gof, webrtc::kMaxVp9FramesInGof);
  size_t length = 1;  // V
  ...
  for (size_t i = 0; i < hdr.gof.num_frames_in_gof; ++i) {
    RTC_DCHECK_LE(hdr.gof.num_ref_pics[i], webrtc::kMaxVp9RefPics);
    length += hdr.gof.num_ref_pics[i];  // R times
  }
  return length;
}

size_t PayloadDescriptorLengthMinusSsData(const webrtc::RTPVideoHeaderVP9& hdr) {
...
//      +-+-+-+-+-+-+-+-+
//
bool WritePictureId(const webrtc::RTPVideoHeaderVP9& vp9,
...
//      +-+-+-+-+-+-+-+-+
//
bool WriteLayerInfoCommon(const webrtc::RTPVideoHeaderVP9& vp9,
                          rtc::BitBufferWriter* writer) {
...
//      +-+-+-+-+-+-+-+-+
//
bool WriteLayerInfoNonFlexibleMode(const webrtc::RTPVideoHeaderVP9& vp9,
                                   rtc::BitBufferWriter* writer) {
  RETURN_FALSE_ON_ERROR(writer->WriteUInt8(Tl0PicIdxField(vp9, 0)));
  return true;
}

bool WriteLayerInfo(const webrtc::RTPVideoHeaderVP9& vp9,
                    rtc::BitBufferWriter* writer) {
...
//                                                current P_DIFF.
//
bool WriteRefIndices(const webrtc::RTPVideoHeaderVP9& vp9,
                     rtc::BitBufferWriter* writer) {
  if (!PictureIdPresent(vp9) || vp9.num_ref_pics == 0 ||
      vp9.num_ref_pics > webrtc::kMaxVp9RefPics) {
    return false;
  }
...
//      +-+-+-+-+-+-+-+-+              -|           -|
//
bool WriteSsData(const webrtc::RTPVideoHeaderVP9& vp9, rtc::BitBufferWriter* writer) {
  RTC_DCHECK_GT(vp9.num_spatial_layers, 0U);
  RTC_DCHECK_LE(vp9.num_spatial_layers, webrtc::kMaxVp9NumberOfSpatialLayers);
  RTC_DCHECK_LE(vp9.gof.num_frames_in_gof, webrtc::kMaxVp9FramesInGof);
...
webrtc::RTPVideoHeaderVP9 RemoveInactiveSpatialLayers(
    const webrtc::RTPVideoHeaderVP9& original_header) {
	webrtc::RTPVideoHeaderVP9 hdr(original_header);
...
RtpPacketizerVp9::RtpPacketizerVp9(rtc::ArrayView<const uint8_t> payload,
                                   ...
                                   const webrtc::RTPVideoHeaderVP9& hdr)
