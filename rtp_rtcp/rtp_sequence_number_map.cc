...
void RtpSequenceNumberMap::InsertPacket(uint16_t sequence_number, Info info) {
  RTC_DCHECK(associations_.size() < 2 ||
             webrtc::AheadOf(associations_.back().sequence_number,
  ...
  if (webrtc::AheadOrAt(sequence_number, associations_.front().sequence_number) &&
      webrtc::AheadOrAt(associations_.back().sequence_number, sequence_number)) {
  ...
  auto cmp = [](const Association& a, uint16_t sequence_number) {
    return webrtc::AheadOf(a.sequence_number, sequence_number);
  };
