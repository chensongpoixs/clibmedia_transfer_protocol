...
RTCPSender::Configuration RTCPSender::Configuration::FromRtpRtcpConfiguration(
  ...
  if (configuration.rtcp_report_interval_ms) {
    result.rtcp_report_interval =
		webrtc::TimeDelta::Millis(configuration.rtcp_report_interval_ms);
  }
...
void RTCPSender::SetRemb(int64_t bitrate_bps, std::vector<uint32_t> ssrcs) {
  ...
  SetNextRtcpSendEvaluationDuration(webrtc::TimeDelta::Zero());
}
...
void RTCPSender::BuildExtendedReports(const RtcpContext& ctx,
  ...
  if (send_video_bitrate_allocation_) {
    ...
    for (int sl = 0; sl < webrtc::kMaxSpatialLayers; ++sl) {
      for (int tl = 0; tl < webrtc::kMaxTemporalStreams; ++tl) {
...
void RTCPSender::PrepareReport(const FeedbackState& feedback_state) {
  ...
  if (generate_report) {
	...
	webrtc::TimeDelta min_interval = report_interval_;

    if (!audio_ && sending_) {
      // Calculate bandwidth for video; 360 / send bandwidth in kbit/s.
      int send_bitrate_kbit = feedback_state.send_bitrate / 1000;
      if (send_bitrate_kbit != 0) {
        min_interval = std::min(webrtc::TimeDelta::Millis(360000 / send_bitrate_kbit),
                                report_interval_);
      }
    }
...
void RTCPSender::SetVideoBitrateAllocation(
    const webrtc::VideoBitrateAllocation& bitrate) {
	webrtc::MutexLock lock(&mutex_rtcp_sender_);
  if (method_ == RtcpMode::kOff) {
    RTC_LOG(LS_WARNING) << "Can't send rtcp if it is disabled.";
    return;
  ...
  // as soon as possible.
  absl::optional<webrtc::VideoBitrateAllocation> new_bitrate =
...
absl::optional<webrtc::VideoBitrateAllocation> RTCPSender::CheckAndUpdateLayerStructure(
    const webrtc::VideoBitrateAllocation& bitrate) const {
  absl::optional<webrtc::VideoBitrateAllocation> updated_bitrate;
  for (size_t si = 0; si < webrtc::kMaxSpatialLayers; ++si) {
    for (size_t ti = 0; ti < webrtc::kMaxTemporalStreams; ++ti) {
      if (!updated_bitrate &&
