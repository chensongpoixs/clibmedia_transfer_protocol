...
static constexpr webrtc::DataRate kDataRateResolution = webrtc::DataRate::KilobitsPerSec(1);
...
class DataRateSerializer {
 public:
  DataRateSerializer(
      uint8_t id,
      std::function<webrtc::DataRate*(libice::NetworkStateEstimate*)> field_getter)
      : id_(id), field_getter_(field_getter) {}

  uint8_t id() const { return id_; }

  void Read(const uint8_t* src, libice::NetworkStateEstimate* target) const {
    int64_t scaled = ByteReader<uint32_t, kFieldValueSize>::ReadBigEndian(src);
    if (scaled == kMaxEncoded) {
      *field_getter_(target) = webrtc::DataRate::PlusInfinity();
    } else {
      *field_getter_(target) = kDataRateResolution * scaled;
    }
  }

  bool Write(const libice::NetworkStateEstimate& src, uint8_t* target) const {
    auto value = *field_getter_(const_cast<libice::NetworkStateEstimate*>(&src));
    ...
    if (value.IsPlusInfinity()) {
      ...
      if (scaled >= kMaxEncoded) {
        scaled = kMaxEncoded;
        RTC_LOG(LS_WARNING) << webrtc::ToString(value) << " is larger than max ("
 ...
 private:
  const uint8_t id_;
  const std::function<webrtc::DataRate*(libice::NetworkStateEstimate*)> field_getter_;
};
...
class RemoteEstimateSerializerImpl : public RemoteEstimateSerializer {
 public:
  ...
  rtc::Buffer Serialize(const libice::NetworkStateEstimate& src) const override {
  ...
  bool Parse(rtc::ArrayView<const uint8_t> src,
             libice::NetworkStateEstimate* target) const override {
...
const RemoteEstimateSerializer* GetRemoteEstimateSerializer() {
  using E = libice::NetworkStateEstimate;
...
void RemoteEstimate::SetEstimate(libice::NetworkStateEstimate estimate) {
  estimate_ = estimate;
