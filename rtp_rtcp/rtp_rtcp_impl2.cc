...
RTCPSender::Configuration AddRtcpSendEvaluationCallback(
    ...
    std::function<void(webrtc::TimeDelta)> send_evaluation_callback) {
  config.schedule_next_rtcp_send_evaluation_function =
      std::move(send_evaluation_callback);
  return config;
}

int DelayMillisForDuration(webrtc::TimeDelta duration) {
...
ModuleRtpRtcpImpl2::ModuleRtpRtcpImpl2(const Configuration& configuration)
  ...
  rtt_update_task_ = webrtc::RepeatingTaskHandle::DelayedStart(
...
std::unique_ptr<ModuleRtpRtcpImpl2> ModuleRtpRtcpImpl2::Create(
  ...
  RTC_DCHECK(webrtc::TaskQueueBase::Current());
...
void ModuleRtpRtcpImpl2::set_rtt_ms(int64_t rtt_ms) {
  RTC_DCHECK_RUN_ON(worker_queue_);
  {
	  webrtc::MutexLock lock(&mutex_rtt_);
    rtt_ms_ = rtt_ms;
  }
  if (rtp_sender_) {
...
void ModuleRtpRtcpImpl2::PeriodicUpdate() {
  ...
  webrtc::Timestamp check_since = clock_->CurrentTime() - kRttUpdateInterval;
...
void ModuleRtpRtcpImpl2::MaybeSendRtcpAtOrAfterTimestamp(
	webrtc::Timestamp execution_time) {
...
void ModuleRtpRtcpImpl2::ScheduleRtcpSendEvaluation(webrtc::TimeDelta duration) {
  ...
  if (duration.IsZero()) {
    worker_queue_->PostTask(ToQueuedTask(task_safety_, [this] {
	  ...
	  webrtc::Timestamp execution_time = clock_->CurrentTime() + duration;
