...
#include "libmedia_transfer_protocol/rtp_rtcp/rtcp_transceiver.h"
...
RtcpTransceiver::~RtcpTransceiver() {
  ...
  task_queue_->PostTask(
      webrtc::ToQueuedTask([rtcp_transceiver = std::move(rtcp_transceiver)] {
...
void RtcpTransceiver::Stop(std::function<void()> on_destroyed) {
  RTC_DCHECK(rtcp_transceiver_);
  auto rtcp_transceiver = std::move(rtcp_transceiver_);
  task_queue_->PostTask(webrtc::ToQueuedTask(
...
void RtcpTransceiver::AddMediaReceiverRtcpObserver(
  ...
  task_queue_->PostTask(webrtc::ToQueuedTask([ptr, remote_ssrc, observer] {
...
void RtcpTransceiver::RemoveMediaReceiverRtcpObserver(
  ...
  task_queue_->PostTask(webrtc::ToQueuedTask(std::move(remove), std::move(on_removed)));
}
...
void RtcpTransceiver::SetReadyToSend(bool ready) {
  ...
  RtcpTransceiverImpl* ptr = rtcp_transceiver_.get();
  task_queue_->PostTask(
	  webrtc::ToQueuedTask([ptr, ready] { ptr->SetReadyToSend(ready); }));
}
...
void RtcpTransceiver::ReceivePacket(rtc::CopyOnWriteBuffer packet) {
  ...
  task_queue_->PostTask(
	  webrtc::ToQueuedTask([ptr, packet, now] { ptr->ReceivePacket(packet, now); }));
}

void RtcpTransceiver::SendCompoundPacket() {
  RTC_CHECK(rtcp_transceiver_);
  RtcpTransceiverImpl* ptr = rtcp_transceiver_.get();
  task_queue_->PostTask(webrtc::ToQueuedTask([ptr] { ptr->SendCompoundPacket(); }));
}
...
void RtcpTransceiver::SetRemb(int64_t bitrate_bps,
  ...
  RtcpTransceiverImpl* ptr = rtcp_transceiver_.get();
  task_queue_->PostTask(
	  webrtc::ToQueuedTask([ptr, bitrate_bps, ssrcs = std::move(ssrcs)]() mutable {
...
void RtcpTransceiver::UnsetRemb() {
  RTC_CHECK(rtcp_transceiver_);
  RtcpTransceiverImpl* ptr = rtcp_transceiver_.get();
  task_queue_->PostTask(webrtc::ToQueuedTask([ptr] { ptr->UnsetRemb(); }));
}
...
void RtcpTransceiver::SendCombinedRtcpPacket(
  ...
  RtcpTransceiverImpl* ptr = rtcp_transceiver_.get();
  task_queue_->PostTask(
	  webrtc::ToQueuedTask([ptr, rtcp_packets = std::move(rtcp_packets)]() mutable {
...
void RtcpTransceiver::SendPictureLossIndication(uint32_t ssrc) {
  ...
  RtcpTransceiverImpl* ptr = rtcp_transceiver_.get();
  task_queue_->PostTask(
	  webrtc::ToQueuedTask([ptr, ssrc] { ptr->SendPictureLossIndication(ssrc); }));
}
...
void RtcpTransceiver::SendFullIntraRequest(std::vector<uint32_t> ssrcs,
  ...
  RtcpTransceiverImpl* ptr = rtcp_transceiver_.get();
  task_queue_->PostTask(
	  webrtc::ToQueuedTask([ptr, ssrcs = std::move(ssrcs), new_request] {
