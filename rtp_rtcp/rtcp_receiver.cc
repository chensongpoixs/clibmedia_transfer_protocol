...
#include "libmedia_transfer_protocol/rtp_rtcp/rtcp_packet/loss_notification.h"
...
#include "libmedia_transfer_protocol/rtp_rtcp/rtcp_packet/remote_estimate.h"
...
#include "libmedia_transfer_protocol/rtp_rtcp/rtp_rtcp_config.h"
#include "libmedia_transfer_protocol/rtp_rtcp/rtp_rtcp_impl2.h"
#include "libmedia_transfer_protocol/rtp_rtcp/time_util.h"
#include "libmedia_transfer_protocol/rtp_rtcp/tmmbr_help.h"
...
bool ResetTimestampIfExpired(const webrtc::Timestamp now,
	webrtc::Timestamp& timestamp,
	webrtc::TimeDelta interval) {
  if (timestamp.IsInfinite() ||
      now <= timestamp + interval * kRrTimeoutIntervals) {
    return false;
  }

  timestamp = webrtc::Timestamp::PlusInfinity();
  return true;
}
...
RTCPReceiver::RTCPReceiver(const RtpRtcpInterface::Configuration& config,
    ...
    : clock_(config.clock),
      ...
      report_interval_(config.rtcp_report_interval_ms > 0
                           ? webrtc::TimeDelta::Millis(config.rtcp_report_interval_ms)
...
int64_t RTCPReceiver::LastReceivedReportBlockMs() const {
	webrtc::MutexLock lock(&rtcp_receiver_lock_);
...
void RTCPReceiver::RttStats::AddRtt(webrtc::TimeDelta rtt) {
  last_rtt_ = rtt;
  if (rtt < min_rtt_) {
    min_rtt_ = rtt;
  }
...
absl::optional<webrtc::TimeDelta> RTCPReceiver::OnPeriodicRttUpdate(
	webrtc::Timestamp newer_than,
    bool sending) {
  // Running on the worker thread (same as construction thread).
  absl::optional<webrtc::TimeDelta> rtt;
  ...
  if (sending) {
    ...
    if (last_received_rb_.IsInfinite() || last_received_rb_ > newer_than) {
		webrtc::TimeDelta max_rtt = webrtc::TimeDelta::MinusInfinity();
    ...
    if (GetAndResetXrRrRtt(&rtt_ms)) {
      rtt.emplace(webrtc::TimeDelta::Millis(rtt_ms));
    }
  }
...
void RTCPReceiver::HandleReceiverReport(const rtcp::CommonHeader& rtcp_block,
  ...
  for (const rtcp::ReportBlock& report_block : receiver_report.report_blocks())
    HandleReportBlock(report_block, packet_information, remote_ssrc);
}
...
void RTCPReceiver::HandleReportBlock(const rtcp::ReportBlock& report_block,
  ...
  if (send_time_ntp != 0) {
    ...
    if (report_block.source_ssrc() == main_ssrc_) {
      rtts_[remote_ssrc].AddRtt(webrtc::TimeDelta::Millis(rtt_ms));
    }
...
void RTCPReceiver::HandleSdes(const rtcp::CommonHeader& rtcp_block,
                              PacketInformation* packet_information) {
  rtcp::Sdes sdes;
...
void RTCPReceiver::HandleBye(const rtcp::CommonHeader& rtcp_block) {
  rtcp::Bye bye;
...
void RTCPReceiver::HandleXr(const rtcp::CommonHeader& rtcp_block,
                            PacketInformation* packet_information,
                            bool& contains_dlrr,
                            uint32_t& ssrc) {
...
void RTCPReceiver::HandleXrDlrrReportBlock(uint32_t sender_ssrc,
  ...
  non_sender_rtts_[sender_ssrc].Update(webrtc::TimeDelta::Millis(xr_rr_rtt_ms_));
}
...
void RTCPReceiver::HandlePli(const rtcp::CommonHeader& rtcp_block,
                             PacketInformation* packet_information) {
  rtcp::Pli pli;
...
void RTCPReceiver::HandleTmmbr(const rtcp::CommonHeader& rtcp_block,
                               PacketInformation* packet_information) {
  rtcp::Tmmbr tmmbr;
...
void RTCPReceiver::HandleTmmbn(const rtcp::CommonHeader& rtcp_block,
                               PacketInformation* packet_information) {
  rtcp::Tmmbn tmmbn;
...
void RTCPReceiver::HandleSrReq(const rtcp::CommonHeader& rtcp_block,
                               PacketInformation* packet_information) {
...
void RTCPReceiver::HandlePsfbApp(const rtcp::CommonHeader& rtcp_block,
                                 PacketInformation* packet_information) {
  {
    rtcp::Remb remb;
...
void RTCPReceiver::HandleFir(const rtcp::CommonHeader& rtcp_block,
                             PacketInformation* packet_information) {
  rtcp::Fir fir;
...
bool RTCPReceiver::RtcpRrTimeoutLocked(webrtc::Timestamp now) {
  return ResetTimestampIfExpired(now, last_received_rb_, report_interval_);
}

bool RTCPReceiver::RtcpRrSequenceNumberTimeoutLocked(webrtc::Timestamp now) {
