...
#include "libmedia_transfer_protocol/rtp_rtcp/rtp_sender_audio.h"
...
const char* FrameTypeToString(webrtc::AudioFrameType frame_type) {
  switch (frame_type) {
    case webrtc::AudioFrameType::kEmptyFrame:
      return "empty";
    case webrtc::AudioFrameType::kAudioFrameSpeech:
      return "audio_speech";
    case webrtc::AudioFrameType::kAudioFrameCN:
      return "audio_cn";
  }
...
RTPSenderAudio::RTPSenderAudio(webrtc::Clock* clock, RTPSender* rtp_sender)
    : clock_(clock),
...
int32_t RTPSenderAudio::RegisterAudioPayload(absl::string_view payload_name,
  ...
  if (absl::EqualsIgnoreCase(payload_name, "cn")) {
	  webrtc::MutexLock lock(&send_audio_mutex_);
...
bool RTPSenderAudio::MarkerBit(webrtc::AudioFrameType frame_type, int8_t payload_type) {
	webrtc::MutexLock lock(&send_audio_mutex_);
  ...
  if (last_payload_type_ != payload_type) {
    ...
    if (last_payload_type_ == -1) {
      if (frame_type != webrtc::AudioFrameType::kAudioFrameCN) {
...
bool RTPSenderAudio::SendAudio(webrtc::AudioFrameType frame_type,
                               int8_t payload_type,
...
bool RTPSenderAudio::SendAudio(webrtc::AudioFrameType frame_type,
  ...
  if (dtmf_event_is_on_) {
    if (frame_type == webrtc::AudioFrameType::kEmptyFrame) {
  ...
  packet->SetExtension<AudioLevel>(
      frame_type == webrtc::AudioFrameType::kAudioFrameSpeech, audio_level_dbov);
  ...
  if (absolute_capture_timestamp_ms > 0) {
    ...
    auto absolute_capture_time = absolute_capture_time_sender_.OnSendPacket(
		...
		webrtc::Int64MsToUQ32x32(clock_->ConvertTimestampToNtpTimeInMilliseconds(
