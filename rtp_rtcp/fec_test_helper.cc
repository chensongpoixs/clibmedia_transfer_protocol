...
#include "libmedia_transfer_protocol/rtp_rtcp/fec_test_helper.h"
...
ForwardErrorCorrection::PacketList MediaPacketGenerator::ConstructMediaPackets(
  ...
  for (int i = 0; i < num_media_packets; ++i) {
	...
	libmedia_transfer_protocol::ByteWriter<uint16_t>::WriteBigEndian(&data[2], seq_num);
	libmedia_transfer_protocol::ByteWriter<uint32_t>::WriteBigEndian(&data[4], time_stamp);
	libmedia_transfer_protocol::ByteWriter<uint32_t>::WriteBigEndian(&data[8], ssrc_);
