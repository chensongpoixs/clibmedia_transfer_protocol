...
#ifndef _C_MODULES_RTP_RTCP_SOURCE_RTCP_RECEIVER_H_
#define _C_MODULES_RTP_RTCP_SOURCE_RTCP_RECEIVER_H_
...
#include "libmedia_transfer_protocol/rtp_rtcp/rtcp_nack_stats.h"
...
#include "libmedia_transfer_protocol/rtp_rtcp/rtp_rtcp_interface.h"
 ...
 public:
  ...
  class NonSenderRttStats {
   public:
    ...
    ~NonSenderRttStats() = default;
    void Update(webrtc::TimeDelta non_sender_rtt_seconds) {
    ...
    absl::optional<webrtc::TimeDelta> round_trip_time() const {
      return round_trip_time_;
    }
    // https://www.w3.org/TR/webrtc-stats/#dom-rtcremoteoutboundrtpstreamstats-totalroundtriptime
	webrtc::TimeDelta total_round_trip_time() const { return total_round_trip_time_; }
   ...
   private:
    absl::optional<webrtc::TimeDelta> round_trip_time_;
	webrtc::TimeDelta total_round_trip_time_ = webrtc::TimeDelta::Zero();
  ...
  absl::optional<webrtc::TimeDelta> OnPeriodicRttUpdate(webrtc::Timestamp newer_than,
                                                bool sending);
 ...
 private:
  ...
  class CustomSequenceChecker : public webrtc::SequenceChecker {
   public:
    ...
    bool IsCurrent() const {
      if (disable_checks_)
        return true;
      return webrtc::SequenceChecker::IsCurrent();
    }
  ...
  class RttStats {
   public:
    ...
    void AddRtt(webrtc::TimeDelta rtt);

    webrtc::TimeDelta last_rtt() const { return last_rtt_; }
    webrtc::TimeDelta min_rtt() const { return min_rtt_; }
    webrtc::TimeDelta max_rtt() const { return max_rtt_; }
    webrtc::TimeDelta average_rtt() const { return sum_rtt_ / num_rtts_; }

   private:
    webrtc::TimeDelta last_rtt_ =webrtc:: TimeDelta::Zero();
    webrtc::TimeDelta min_rtt_ = webrtc::TimeDelta::PlusInfinity();
    webrtc::TimeDelta max_rtt_ = webrtc::TimeDelta::MinusInfinity();
    webrtc::TimeDelta sum_rtt_ = webrtc::TimeDelta::Zero();
    size_t num_rtts_ = 0;
  };
  ...
  bool RtcpRrTimeoutLocked(webrtc::Timestamp now)
      RTC_EXCLUSIVE_LOCKS_REQUIRED(rtcp_receiver_lock_);

  bool RtcpRrSequenceNumberTimeoutLocked(webrtc::Timestamp now)
      RTC_EXCLUSIVE_LOCKS_REQUIRED(rtcp_receiver_lock_);
  ...
  webrtc::NtpTime remote_sender_ntp_time_ RTC_GUARDED_BY(rtcp_receiver_lock_);
  uint32_t remote_sender_rtp_time_ RTC_GUARDED_BY(rtcp_receiver_lock_);
  // When did we receive the last send report.
  webrtc::NtpTime last_received_sr_ntp_ RTC_GUARDED_BY(rtcp_receiver_lock_);
  ...
  webrtc::flat_map<uint32_t, std::list<RrtrInformation>::iterator>
  ...
  webrtc::flat_map<uint32_t, TmmbrInformation> tmmbr_infos_
      RTC_GUARDED_BY(rtcp_receiver_lock_);

  // Round-Trip Time per remote sender ssrc.
  webrtc::flat_map<uint32_t, RttStats> rtts_ RTC_GUARDED_BY(rtcp_receiver_lock_);
  // Non-sender Round-trip time per remote ssrc.
  webrtc::flat_map<uint32_t, NonSenderRttStats> non_sender_rtts_
      RTC_GUARDED_BY(rtcp_receiver_lock_);

  // Report blocks per local source ssrc.
  webrtc::flat_map<uint32_t, ReportBlockData> received_report_blocks_
      RTC_GUARDED_BY(rtcp_receiver_lock_);
  webrtc::flat_map<uint32_t, LastFirStatus> last_fir_
      RTC_GUARDED_BY(rtcp_receiver_lock_);

  // The last time we received an RTCP Report block for this module.
  webrtc::Timestamp last_received_rb_ RTC_GUARDED_BY(rtcp_receiver_lock_) =
	  webrtc::Timestamp::PlusInfinity();

  // The time we last received an RTCP RR telling we have successfully
  // delivered RTP packet to the remote side.
  webrtc::Timestamp last_increased_sequence_number_ = webrtc::Timestamp::PlusInfinity();
