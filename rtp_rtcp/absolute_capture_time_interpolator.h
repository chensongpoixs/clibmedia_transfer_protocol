...
#ifndef _C_MODULES_RTP_RTCP_SOURCE_ABSOLUTE_CAPTURE_TIME_INTERPOLATOR_H_
#define _C_MODULES_RTP_RTCP_SOURCE_ABSOLUTE_CAPTURE_TIME_INTERPOLATOR_H_
...
class AbsoluteCaptureTimeInterpolator {
 public:
  ...
  explicit AbsoluteCaptureTimeInterpolator(webrtc::Clock* clock);
 ...
 private:
  ...
  bool ShouldInterpolateExtension(webrtc::Timestamp receive_time,
                                  uint32_t source,
                                  uint32_t rtp_timestamp,
  ...
  webrtc::Timestamp last_receive_time_ RTC_GUARDED_BY(mutex_);
