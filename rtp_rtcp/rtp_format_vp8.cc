...
RtpPacketizerVp8::RtpPacketizerVp8(rtc::ArrayView<const uint8_t> payload,
                                   ...
                                   const webrtc::RTPVideoHeaderVP8& hdr_info)
...
RtpPacketizerVp8::RawHeader RtpPacketizerVp8::BuildHeader(
    const webrtc::RTPVideoHeaderVP8& header) {
  ...
  RawHeader result;
  bool tid_present = header.temporalIdx != webrtc::kNoTemporalIdx;
  bool keyid_present = header.keyIdx != webrtc::kNoKeyIdx;
  bool tl0_pid_present = header.tl0PicIdx != webrtc::kNoTl0PicIdx;
  bool pid_present = header.pictureId != webrtc::kNoPictureId;
  uint8_t x_field = 0;
  if (pid_present)
    x_field |= kIBit;
  if (tl0_pid_present)
