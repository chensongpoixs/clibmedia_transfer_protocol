...
#include "libmedia_transfer_protocol/rtp_rtcp/flexfec_header_reader_writer.h"
#include "libmedia_transfer_protocol/rtp_rtcp/forward_error_correction_internal.h"
#include "libmedia_transfer_protocol/rtp_rtcp/ulpfec_header_reader_writer.h"
...
bool ForwardErrorCorrection::SortablePacket::LessThan::operator()(
  ...
  return webrtc::IsNewerSequenceNumber(second->seq_num, first->seq_num);
}
...
void ForwardErrorCorrection::InsertPacket(
  ...
  if (!received_fec_packets_.empty() &&
    ...
    while (it != received_fec_packets_.end()) {
      uint16_t seq_num_diff = webrtc::MinDiff(received_packet.seq_num, (*it)->seq_num);
...
bool ForwardErrorCorrection::IsOldFecPacket(
  ...
  return (webrtc::MinDiff(back_recovered_seq_num, last_protected_seq_num) >
  ...
  if (recovered_packets->size() == max_media_packets) {
    ...
    if (received_packet.ssrc == back_recovered_packet->ssrc) {
      const unsigned int seq_num_diff =
          webrtc::MinDiff(received_packet.seq_num, back_recovered_packet->seq_num);
