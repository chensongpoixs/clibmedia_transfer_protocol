...
#include "libmedia_transfer_protocol/rtp_rtcp/rtp_generic_frame_descriptor_extension.h"
...
#include "libmedia_transfer_protocol/rtp_rtcp/rtp_video_layers_allocation_extension.h"
...
bool MinimizeDescriptor(RTPVideoHeader* video_header) {
  if (auto* vp8 =
          absl::get_if<webrtc::RTPVideoHeaderVP8>(&video_header->video_type_header)) {
...
absl::optional<webrtc::VideoPlayoutDelay> LoadVideoPlayoutDelayOverride(
  ...
  webrtc::FieldTrialOptional<int> playout_delay_max_ms("max_ms", absl::nullopt);
...
RTPSenderVideo::RTPSenderVideo(const Config& config)
    : rtp_sender_(config.rtp_sender),
      ...
      packetization_overhead_bitrate_(1000, webrtc::RateStatistics::kBpsScale),
...
bool RTPSenderVideo::SendVideo(
  ...
  const bool use_fec = fec_type_.has_value() &&
                       (temporal_id == 0 || temporal_id == webrtc::kNoTemporalIdx);
  ...
  if (frame_encryptor_ != nullptr) {
    const size_t max_ciphertext_size =
        frame_encryptor_->GetMaxCiphertextByteSize(MEDIA_TYPE_VIDEO,
                                                   payload.size());
    ...
    if (frame_encryptor_->Encrypt(
            libmedia_transfer_protocol::MEDIA_TYPE_VIDEO, first_packet->Ssrc(), additional_data,
...
bool RTPSenderVideo::AllowRetransmission(
  ...
  if (temporal_id == webrtc::kNoTemporalIdx)
    return true;
...
uint8_t RTPSenderVideo::GetTemporalId(const RTPVideoHeader& header) {
  struct TemporalIdGetter {
    uint8_t operator()(const webrtc::RTPVideoHeaderVP8& vp8) { return vp8.temporalIdx; }
    uint8_t operator()(const webrtc::RTPVideoHeaderVP9& vp9) {
      return vp9.temporal_idx;
    }
    uint8_t operator()(const webrtc::RTPVideoHeaderH264&) { return webrtc::kNoTemporalIdx; }
    uint8_t operator()(const RTPVideoHeaderLegacyGeneric&) {
      return webrtc::kNoTemporalIdx;
    }
    uint8_t operator()(const absl::monostate&) { return webrtc::kNoTemporalIdx; }
  };
...
bool RTPSenderVideo::UpdateConditionalRetransmit(
  ...
  if (temporal_id != webrtc::kNoTemporalIdx && temporal_id > 0) {
