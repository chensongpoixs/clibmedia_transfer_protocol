...
#ifndef _C_MODULES_RTP_RTCP_SOURCE_RTCP_SENDER_H_
#define _C_MODULES_RTP_RTCP_SOURCE_RTCP_SENDER_H_
...
class RTCPSender final {
 public:
  ...
  void SetTargetBitrate(unsigned int target_bitrate)
      RTC_LOCKS_EXCLUDED(mutex_rtcp_sender_);
  void SetVideoBitrateAllocation(const webrtc::VideoBitrateAllocation& bitrate)
      RTC_LOCKS_EXCLUDED(mutex_rtcp_sender_);
 ...
 private:
  ...
  const std::function<void(webrtc::TimeDelta)>
      schedule_next_rtcp_send_evaluation_function_;

  mutable webrtc::Mutex mutex_rtcp_sender_;
  ...
  webrtc::VideoBitrateAllocation video_bitrate_allocation_
      RTC_GUARDED_BY(mutex_rtcp_sender_);
  ...
  absl::optional<webrtc::VideoBitrateAllocation> CheckAndUpdateLayerStructure(
