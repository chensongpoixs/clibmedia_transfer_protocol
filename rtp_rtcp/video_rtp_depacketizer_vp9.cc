...
//      +-+-+-+-+-+-+-+-+
//
void ParsePictureId(webrtc::BitstreamReader& parser, webrtc::RTPVideoHeaderVP9* vp9) {
  if (parser.ReadBit()) {  // m_bit
    vp9->picture_id = parser.ReadBits(15);
    vp9->max_picture_id = webrtc::kMaxTwoBytePictureId;
  } else {
    vp9->picture_id = parser.ReadBits(7);
    vp9->max_picture_id = webrtc::kMaxOneBytePictureId;
  }
}
...
//      +-+-+-+-+-+-+-+-+
//
void ParseLayerInfo(webrtc::BitstreamReader& parser, webrtc::RTPVideoHeaderVP9* vp9) {
  ...
  if (vp9->spatial_idx >= webrtc::kMaxSpatialLayers) {
    parser.Invalidate();
    return;
  }
...
//                                                current P_DIFF.
//
void ParseRefIndices(webrtc::BitstreamReader& parser, webrtc::RTPVideoHeaderVP9* vp9) {
  if (vp9->picture_id == webrtc::kNoPictureId) {
    parser.Invalidate();
    return;
  }
  ...
  bool n_bit;
  do {
    if (vp9->num_ref_pics == webrtc::kMaxVp9RefPics) {
      parser.Invalidate();
      return;
    }
...
//      +-+-+-+-+-+-+-+-+              -|           -|
//
void ParseSsData(webrtc::BitstreamReader& parser, webrtc::RTPVideoHeaderVP9* vp9) {
...
int VideoRtpDepacketizerVp9::ParseRtpPayload(
  ...
  webrtc::BitstreamReader parser(rtp_payload);
