...
#include "libmedia_transfer_protocol/rtp_rtcp/fec_private_tables_bursty.h"
#include "libmedia_transfer_protocol/rtp_rtcp/fec_private_tables_random.h"
...
PacketMaskTable::PacketMaskTable(webrtc::FecMaskType fec_mask_type,
...
const uint8_t* PacketMaskTable::PickTable(webrtc::FecMaskType fec_mask_type,
                                          int num_media_packets) {
  RTC_DCHECK_GE(num_media_packets, 0);
  RTC_DCHECK_LE(static_cast<size_t>(num_media_packets), kUlpfecMaxMediaPackets);

  if (fec_mask_type != webrtc::kFecMaskRandom &&
      num_media_packets <=
