...
#ifndef _C_MODULES_RTP_RTCP_SOURCE_RTP_RTCP_IMPL2_H_
#define _C_MODULES_RTP_RTCP_SOURCE_RTP_RTCP_IMPL2_H_
...
#include "libmedia_transfer_protocol/rtp_rtcp/rtp_rtcp_defines.h"  // RTCPPacketType
#include "libmedia_transfer_protocol/rtp_rtcp/packet_sequencer.h"
...
#include "libmedia_transfer_protocol/rtp_rtcp/rtcp_sender.h"
#include "libmedia_transfer_protocol/rtp_rtcp/rtp_packet_history.h"
#include "libmedia_transfer_protocol/rtp_rtcp/rtp_packet_to_send.h"
#include "libmedia_transfer_protocol/rtp_rtcp/rtp_sender.h"
#include "libmedia_transfer_protocol/rtp_rtcp/rtp_sender_egress.h"
...
namespace libmedia_transfer_protocol {
	...
	public:
		...
		bool TrySendPacket(RtpPacketToSend* packet,
			const libice::PacedPacketInfo& pacing_info) override;

		void SetFecProtectionParams(const webrtc::FecProtectionParams& delta_params,
	...
	private:
		...
		struct RtpSenderContext {
			...
			webrtc::SequenceChecker sequencing_checker;
		...
		void ScheduleRtcpSendEvaluation(webrtc::TimeDelta duration);
		...
		// is resolved.
		void MaybeSendRtcpAtOrAfterTimestamp(webrtc::Timestamp execution_time)
			RTC_RUN_ON(worker_queue_);

		// Schedules a call to MaybeSendRtcpAtOrAfterTimestamp delayed by `duration`.
		void ScheduleMaybeSendRtcpAtOrAfterTimestamp(webrtc::Timestamp execution_time,
			webrtc::TimeDelta duration);

		webrtc::TaskQueueBase* const worker_queue_;
		RTC_NO_UNIQUE_ADDRESS webrtc::SequenceChecker rtcp_thread_checker_;
		...
		webrtc::RemoteBitrateEstimator* const remote_bitrate_;

		RtcpRttStats* const rtt_stats_;
		webrtc::RepeatingTaskHandle rtt_update_task_ RTC_GUARDED_BY(worker_queue_);

		// The processed RTT from RtcpRttStats.
		mutable webrtc::Mutex mutex_rtt_;
		int64_t rtt_ms_ RTC_GUARDED_BY(mutex_rtt_);

		RTC_NO_UNIQUE_ADDRESS webrtc::ScopedTaskSafety task_safety_;
	};
