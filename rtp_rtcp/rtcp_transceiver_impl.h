...
#ifndef _C_MODULES_RTP_RTCP_SOURCE_RTCP_TRANSCEIVER_IMPL_H_
#define _C_MODULES_RTP_RTCP_SOURCE_RTCP_TRANSCEIVER_IMPL_H_
...
#include "libmedia_transfer_protocol/rtp_rtcp/rtcp_transceiver_config.h"
...
class RtcpTransceiverImpl {
 public:
  ...
  void ReceivePacket(rtc::ArrayView<const uint8_t> packet, webrtc::Timestamp now);
 ...
 private:
  ...
  void HandleDlrr(const rtcp::Dlrr& dlrr, webrtc::Timestamp now);
  ...
  std::vector<rtcp::ReportBlock> CreateReportBlocks(webrtc::Timestamp now);
  ...
  // needed.
  webrtc::flat_map<uint32_t, RemoteSenderState> remote_senders_;
  webrtc::RepeatingTaskHandle periodic_task_handle_;
};
