...
#ifndef _C_MODULES_RTP_RTCP_SOURCE_RECEIVE_STATISTICS_IMPL_H_
#define _C_MODULES_RTP_RTCP_SOURCE_RECEIVE_STATISTICS_IMPL_H_
...
class StreamStatisticianImpl : public StreamStatisticianImplInterface {
 ...
 private:
  ...
  webrtc::SequenceNumberUnwrapper seq_unwrapper_;
...
class StreamStatisticianLocked : public StreamStatisticianImplInterface {
 public:
  ...
  RtpReceiveStats GetStats() const override {
	  webrtc::MutexLock lock(&stream_lock_);
 ...
 private:
  mutable webrtc::Mutex stream_lock_;
...
class ReceiveStatisticsImpl : public ReceiveStatistics {
 ...
 private:
  ...
  int max_reordering_threshold_;
  webrtc::flat_map<uint32_t /*ssrc*/, std::unique_ptr<StreamStatisticianImplInterface>>
      statisticians_;
};
...
class ReceiveStatisticsLocked : public ReceiveStatistics {
 public:
  ...
  std::vector<rtcp::ReportBlock> RtcpReportBlocks(size_t max_blocks) override {
	  webrtc::MutexLock lock(&receive_statistics_lock_);
 ...
 private:
  mutable webrtc::Mutex receive_statistics_lock_;
