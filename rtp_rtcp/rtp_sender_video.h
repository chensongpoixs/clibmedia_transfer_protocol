...
#ifndef _C_MODULES_RTP_RTCP_SOURCE_RTP_SENDER_VIDEO_H_
#define _C_MODULES_RTP_RTCP_SOURCE_RTP_SENDER_VIDEO_H_
...
class RTPSenderVideo {
 public:
  ...
  struct Config {
	...
	webrtc::TaskQueueBase* send_transport_queue = nullptr;
  };
 ...
 private:
  struct TemporalLayerStats {
    ...
    // low frame rates.
	webrtc::RateStatistics frame_rate_fp1000s;
  ...
  webrtc::RateStatistics packetization_overhead_bitrate_ RTC_GUARDED_BY(stats_mutex_);

  std::map<int, TemporalLayerStats> frame_stats_by_temporal_layer_
      RTC_GUARDED_BY(stats_mutex_);

  webrtc::OneTimeEvent first_frame_sent_;
