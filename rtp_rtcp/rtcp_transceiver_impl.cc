...
#include "libmedia_transfer_protocol/rtp_rtcp/rtcp_transceiver_impl.h"
...
struct SenderReportTimes {
  webrtc::Timestamp local_received_time;
  webrtc::NtpTime remote_sent_time;
};
...
void RtcpTransceiverImpl::HandlePayloadSpecificFeedback(
  ...
  if (remb.Parse(rtcp_packet_header)) {
    config_.network_link_observer->OnReceiverEstimatedMaxBitrate(
        now, webrtc::DataRate::BitsPerSec(remb.bitrate_bps()));
  }
}
...
void RtcpTransceiverImpl::HandleDlrr(const rtcp::Dlrr& dlrr, webrtc::Timestamp now) {
  ...
  for (const rtcp::ReceiveTimeInfo& rti : dlrr.sub_blocks()) {
    ...
    int64_t rtt_ms = webrtc::CompactNtpRttToMs(rtt_ntp);
...
void RtcpTransceiverImpl::ProcessReportBlocks(
  ...
  webrtc::TimeDelta rtt_sum = webrtc::TimeDelta::Zero();
  size_t num_rtts = 0;
  uint32_t receive_time_ntp =
      CompactNtp(config_.clock->ConvertTimestampToNtpTime(now));
  for (const rtcp::ReportBlock& report_block : report_blocks) {
    ...
    rtt_sum += webrtc::TimeDelta::Millis(webrtc::CompactNtpRttToMs(rtt_ntp));
    ++num_rtts;
  }
...
void RtcpTransceiverImpl::SchedulePeriodicCompoundPackets(int64_t delay_ms) {
  periodic_task_handle_ = webrtc::RepeatingTaskHandle::DelayedStart(
      config_.task_queue, webrtc::TimeDelta::Millis(delay_ms), [this] {
        RTC_DCHECK(config_.schedule_periodic_compound_packets);
        RTC_DCHECK(ready_to_send_);
        SendPeriodicCompoundPacket();
        return webrtc::TimeDelta::Millis(config_.report_period_ms);
      });
}
...
std::vector<rtcp::ReportBlock> RtcpTransceiverImpl::CreateReportBlocks(
  ...
  for (rtcp::ReportBlock& report_block : report_blocks) {
    ...
    last_delay = webrtc::SaturatedUsToCompactNtp(
